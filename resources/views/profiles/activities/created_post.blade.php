@component('profiles.activities.activity')
    @slot('heading')
        <a href="#" class="font-weight-bold text-dark-75 text-hover-primary">{{$profile->name}}</a>   created a post to : <a href="{{$activity->subject?$activity->subject->title:''}}"> "{{$activity->subject?$activity->subject->title:''}}"</a>
    @endslot
    @slot('body')
        <!--begin::Info-->
